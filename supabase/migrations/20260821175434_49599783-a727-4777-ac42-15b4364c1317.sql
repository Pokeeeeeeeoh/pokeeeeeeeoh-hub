...
BEGIN
  ...
  IF gateway_key IS NULL THEN
    RAISE WARNING 'Google Calendar retry skipped: gateway credential missing';
    RETURN;
  END IF;
  ...
  FOR appointment_record IN
    ...
    WHERE br.status='booked' AND a.start_time>=now()-interval '1 day'
  ...
  LOOP
    UPDATE public.appointments SET google_sync_status='syncing',google_sync_attempts=google_sync_attempts+1,google_sync_last_attempt_at=now() WHERE id=appointment_record.id;
    PERFORM net.http_post(
      url := 'https://wtrmrhfjrxdfkqpxteuq.supabase.co/functions/v1/sync-gcal-event',
      headers := jsonb_build_object('Content-Type','application/json','apikey',gateway_key,'Authorization','Bearer '||gateway_key,'X-Internal-Service-Key',gateway_key),
